...
First, set up MySQL by running the setup script:

```bash
./scripts/setup_database.sh
```

This will install MySQL (if needed), create the database, and set up the user. Then start the server:
...
- `-mysql-user`: MySQL username (default: `coffee_user`)
- `-mysql-password`: MySQL password (default: `coffee_pass123`)
